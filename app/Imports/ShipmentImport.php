...
class ShipmentImport implements ToModel, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        ...
        if (!empty($row['estimearrival'])) {
            $estimearrival = ExcelDate::excelToDateTimeObject($row['estimearrival']);
        }

        if (!empty($row['estimedelivery'])) {
            $estimedelivery = ExcelDate::excelToDateTimeObject($row['estimedelivery']);
        }
